...
def push_prototypes(modal_idx,
    ...
    protos_idx_in_batch = [-1] * n_prototypes
    for push_iter, (search_batch_input, search_y) in enumerate(dataloader):
        '''
        start_index_of_search keeps track of the index of the image
        assigned to serve as prototype
        '''
        search_batch_input = search_batch_input[:, modal_idx, :, :, :].to(dev)

        start_index_of_search_batch = push_iter * search_batch_size

        update_prototypes_on_batch(modal_idx,
                                   search_batch_input,
                                   ...
                                   proto_bound_boxes,
                                   prototype_class_identity,
                                   protos_index_in_batch=protos_idx_in_batch,
    ...
    if proto_epoch_dir != None and proto_bound_boxes_filename_prefix != None:
        np.save(os.path.join(proto_epoch_dir, str(modal_idx) + \
    ...
    log('\tExecuting push ...') # global_min_fmap_patches

    prototype_update = np.reshape(global_min_fmap_patches,
                                  tuple(prototype_shape))
    dev = prototype_network.prototype_vectors.device
    prototype_network.prototype_vectors.data.copy_(\
            torch.tensor(prototype_update, dtype=torch.float32).to(dev))
    end = time.time()
    log('{0}'.format(proto_bound_boxes))
    log('\tpush time: \t{0}'.format(end -  start))

# update each prototype for current search batch
def update_prototypes_on_batch(modal_idx,
                               search_batch_input,
                               start_index_of_search_batch,
                               prototype_network,
    ...
    proto_dist_ = np.copy(proto_dist_torch.detach().cpu().numpy()) # [b, num_prototypes, h, w]
    ...
    for j in range(n_prototypes):
        ...
        batch_argmin_proto_dist_j = \
            list(np.unravel_index(np.argmin(proto_dist_j, axis=None), proto_dist_j.shape))
        # change the argmin index from the index among images of the target class
        # to the index in the entire search batch
        batch_argmin_proto_dist_j[0] = class_to_img_index_dict[target_class][batch_argmin_proto_dist_j[0]]
        img_index_in_dataset = batch_argmin_proto_dist_j[0] + start_index_of_search_batch

        batch_min_proto_dist_j = np.amin(proto_dist_j)
        if batch_min_proto_dist_j < global_min_proto_dist[j] and \
            img_index_in_dataset not in protos_index_in_batch[:j]:
            protos_index_in_batch[j] = img_index_in_dataset
            # batch_argmin_proto_dist_j = \
            ...
            if dir_for_saving_prototypes is not None:
                if prototype_self_act_filename_prefix is not None:
                    ...
                    np.save(os.path.join(dir_for_saving_prototypes, str(modal_idx) + \
                                         prototype_self_act_filename_prefix + str(j) + '.npy'),
                            proto_act_img_j)
                if prototype_img_filename_prefix is not None:
                    # save the whole image containing the prototype as png
                    plt.imsave(os.path.join(dir_for_saving_prototypes, str(modal_idx) + \
